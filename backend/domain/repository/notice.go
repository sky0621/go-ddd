...
type NoticeCommandRepository interface {
	...
	Create(context.Context, *entity.Notice) error.ApplicationError

	// CreateBatch ... 引数で渡された「お知らせ」データ複数件を作成する。
	CreateBatch(context.Context, []*entity.Notice) error.ApplicationError

	/*
	 * 更新
	 */
	// UpdateByUniqueID ... 引数で渡された「お知らせ」データに含まれるIDを条件に１件を更新する。
	UpdateByUniqueID(context.Context, *entity.Notice) error.ApplicationError

	// UpdateByCondition ... 引数で渡された「お知らせ」データ更新条件に合致する複数の「お知らせ」データを更新する。
	UpdateByCondition(context.Context, *entity.NoticeCommandCondition, *entity.Notice) error.ApplicationError

	// UpdateBatch ... 引数で渡された「お知らせ」データ複数件を更新する。
	UpdateBatch(context.Context, []*entity.Notice) error.ApplicationError

	/*
	 * 削除
	 */
	// DeleteByUniqueID ... 引数で渡されたユニークIDで特定される「お知らせ」データ１件を削除する。
	DeleteByUniqueID(context.Context, enum.UniqueID) error.ApplicationError

	// DeleteByCondition ... 引数で渡された「お知らせ」データ削除条件に合致する複数の「お知らせ」データを削除する。
	DeleteByCondition(context.Context, *entity.NoticeCommandCondition) error.ApplicationError

	// DeleteBatch ... 引数で渡されたユニークIDで特定される「お知らせ」データ複数件を削除する。
	DeleteBatch(context.Context, []enum.UniqueID) error.ApplicationError
}
...
type NoticeQueryRepository interface {
	...
	GetByUniqueID(context.Context, enum.UniqueID) (*entity.Notice, error.ApplicationError)

	// GetByCondition ... 引数で渡された「お知らせ」データ取得条件に合致する複数の「お知らせ」データを返却する。（取得条件が nil の場合は全ての「お知らせ」データを返却する。）
	GetByCondition(context.Context, *entity.NoticeReadCondition) ([]*entity.Notice, error.ApplicationError)
}
